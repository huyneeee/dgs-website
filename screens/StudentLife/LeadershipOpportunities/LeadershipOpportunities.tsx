...
import { LeadershipOpportunity } from '@/services/leadershipOpportunity';
import { useLocale } from 'next-intl';
import React, { use } from 'react';

const LeadershipOpportunities = () => {
  const locale = useLocale();
  const leadershipOpportunitiesData = use(
    LeadershipOpportunity.getLeadershipOpportunity(locale),
  );

  const content = leadershipOpportunitiesData.data;
          ...
          <div className="flex flex-col my-0">
            {content.sections.map((section, index) => {
              return (
                <div
                  key={index}
                  className="flex flex-col md:flex-row gap-6 md:gap-8 py-4 md:py-4"
                >
                  {section.title && (
